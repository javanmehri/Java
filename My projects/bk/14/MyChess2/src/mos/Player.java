...
public class Player implements java.io.Serializable {
    ...
    private Piece piece_toReplace = null;
    ...
    public Board move(Board chessBoard, int from_i, int from_j, int to_i, int to_j)
    {
        ...
        if (piece.isValidMove(chessBoard, to_i,to_j))
        {
            ...
            // Check here if the piece is a Pawn and it's reached to the other side of the board.
            if (piece.getPieceType()== Piece.Type.PAWN && (to_i ==7 || to_i == 0) && shouldReplace)
            {
                //...
                chessBoard.removePiece(to_i,to_j);
                pieceBag.add_toPieceBag(piece);

                chessBoard.occupySpot(piece_toReplace,to_i,to_j);
    ...
    private Piece get_pieceToReplace()
    {
        return piece_toReplace;
    }
    ...
    public void set_PieceToReplace(Piece.Type type)
    {
        Piece piece = pieceBag.pick(type);
        piece_toReplace = piece;
    }
    ...
    public Board undoRemove(Board chessBoard, int from_i, int from_j, int to_i, int to_j)
    {
        ...
        pieceBag.remove_lastPieceFromPieceBag();
        totalMoves --;
    ...
    public Board remove(Board chessBoard, int from_i, int from_j, int to_i, int to_j)
    {
        ...
        if (piece1.isValidRemove(chessBoard, to_i,to_j))
        {
            ...
            pieceBag.add_toPieceBag(piece2);
            lastRemoved = piece2;
            totalMoves ++;
        }
    ...
    {
        return pieceBag.get_pieces();
    }


    public PieceBag get_PieceBag()
    {
        return pieceBag;
    }
