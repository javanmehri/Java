...
public class ScreenBoard {
    ...
    private static void play(Event event)
    {
        ...
        ImageView toImageView = Utl.getImageView(event);
        Piece selectedPiece = Utl.getPiece(GameManager.getTheGame().getBoard() , selectedImage);

        /*if ((Utl.get_XIndex(toImageView) == 7 || Utl.get_XIndex(toImageView) == 0) && selectedPiece.getPieceType()== Piece.Type.PAWN)
        {
            // lunch PawnAtTheEnd.fxml
            GameManager.lunch_PawnAtTheEnd = true;
        }
*/
        GameManager.getTheGame().play(selectedImage, toImageView);
    ...
    private static void update_removedBoard(Player player)
    {
        ...
        for (int i=0 ; i<15; i++)
        {
            if (removedPieces[i]!=null)
            {
                ...
                updateTheTile(removedBoard[i], null, Piece.Color.WHITE);
        }
    }
    ...
    private static void updateTheTile(ImageView imageView, Piece piece, Piece.Color backgroundColor)
    {
        ...
        if (piece == null)
        {
            if (backgroundColor == Piece.Color.WHITE)
                image = Img.Tile_White;
            else
                image = Img.Tile_Black;
        }
        else
            image = Img.getPiece(piece.getPieceType(), piece.getColor(), backgroundColor);
