...
public class MainStageController {
    ...
    private void mouseClick(Event event) throws IOException, CloneNotSupportedException, InterruptedException {
        ...
        if (GameManager.getTheGame().getBoard().isAnyPawnAtTheEnd)
            lunch_PawnAtTheEnd();
    ...
    private void lunch_PawnAtTheEnd()
    {
        GameManager.getTheGame().getBoard().isAnyPawnAtTheEnd=false;
        try{

            FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("PawnAtTheEnd.fxml"));
            Parent root1 = (Parent) fxmlLoader.load();
            Stage stage = new Stage();
            //stage.initModality(Modality.APPLICATION_MODAL);
            stage.initStyle(StageStyle.UNDECORATED);
            stage.setTitle("Replace the Piece !");
            stage.setScene(new Scene(root1));
            stage.setAlwaysOnTop(true);
            stage.setResizable(false);
            //stage.sizeToScene();
